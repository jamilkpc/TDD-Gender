...
dfData <- readRDS('ManifestosClean.RDS')
...
DictCounts <- dfData %>% 
  select(id_municipio, ano, sequencial, sigla_uf, sigla_partido, genero) %>% 
  ...
  mutate(care_count = replace_na(care_count/tokens_count, 0),
         ...
         mulher = case_when(genero=='feminino' ~ 1,
                            genero=='masculino' ~ 0)) %>% 
  drop_na

dfCounts <- DictCounts %>% write_rds('~/Documents/GitHub/TDD-gender/LLM Version/Data/CorrCount.RDS')
